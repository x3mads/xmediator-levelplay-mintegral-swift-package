# XMediatorLevelPlayMintegralAdapterCompatibility

Swift Package Manager package, published by X3M, that lets XMediator
integrations use the LevelPlay `MintegralAdapter` adapter together with
the Google Mobile Ads mediation adapter for the same network.

## Why this package exists

The official LevelPlay adapter package (<https://github.com/ironsource-mobile/LevelPlay-Mintegral-Adapter-Swift-Package.git>) and the Google
mediation adapter package both declare a target named `MintegralAdapter`,
...
`LevelPlayMintegralAdapter`, renaming the conflicting module so both adapters can
coexist.
...
`5.20.0` of this package uses LevelPlay adapter `5.20.0`.
